    ...
    def get_by_id(self, id, args):
        def fn(session):
            ...
            if (data):
                ...
                if (id and args['get_file'] == '1'):
                    data['file_base64'] = str(base64.b64encode(result.file))
        ...
        def fn(session):
            ...
            if (result):
                ...
                if (result.type != 'image/png'
                    and result.type != 'image/jpg'
                    and result.type != 'image/jpeg'
                    and result.type != 'image/gif'):
                    response.headers.set('Content-Disposition', 'inline; filename=' + result.name + '.' + file_ext)
                return response
            else:
